...
export default function CompanyEditPage() {
  ...
  if (!company) {
    return <div className="flex items-center justify-center h-full text-text-secondary text-sm">존재하지 않는 기업입니다.</div>;
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="bg-surface border-b border-border-light px-8 h-[65px] flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-2 text-[15px] text-text-secondary font-semibold">
          <Link href="/admin/dashboard" className="hover:text-text-primary transition-colors">고객사 현황</Link>
          ...
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
          <span className="text-text-primary font-bold">기업 정보 편집</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => router.back()} className="text-sm font-medium text-text-secondary border border-border-light px-4 py-2 rounded-lg hover:bg-surface-hover transition-colors">취소</button>
          <button onClick={saveCompany} className="text-sm font-medium text-text-onBrand bg-brand hover:bg-brand-dark px-4 py-2 rounded-lg transition-colors">저장</button>
        </div>
      </div>

      {/* 본문 */}
      <div className="flex-1 overflow-y-auto px-8 py-6 bg-surface-subtle space-y-5">

        {/* ── 1. 기업 기본 정보 ── */}
        <div className="bg-surface rounded-2xl border border-border-light shadow-sm p-6">
          <h2 className="text-sm font-bold text-text-primary mb-5">기업 기본 정보</h2>
        ...
        <div className="bg-surface rounded-2xl border border-border-light shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-sm font-bold text-text-primary">리더십 유형 정보</h2>
              <p className="text-xs text-text-secondary mt-0.5">다면진단 기반 리더십 유형 정의 및 특징. 파일 업로드로 업데이트합니다.</p>
            </div>
            <div className="flex items-center gap-2">
              {leadershipHistory.length > 0 && (
                <button
                  onClick={() => setHistoryOpen(v => !v)}
                  className="flex items-center gap-1.5 text-xs font-medium text-text-secondary px-3 py-1.5 rounded-lg border border-border-light hover:bg-surface-hover transition-colors"
                >
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-semibold text-text-onBrand bg-brand hover:bg-brand-dark px-3 py-1.5 rounded-lg transition-colors"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="grid grid-cols-2 gap-3">
            {leadershipCurrent.map(info => (
              <div key={info.type} className="rounded-xl border border-border-light p-4 bg-surface-subtle/50">
                <div className="flex items-center gap-2 mb-2">
                  <span className={`text-xs font-semibold px-1.5 h-6 inline-flex items-center rounded ${BADGE_CLS}`}>{info.type}</span>
                </div>
                <p className="text-xs font-semibold text-text-primary mb-1">{info.definition}</p>
                <p className="text-xs text-text-secondary leading-relaxed">{info.characteristics}</p>
              </div>
            ))}
          </div>
          ...
          {historyOpen && leadershipHistory.length > 0 && (
            <div className="mt-4 border-t border-border-light pt-4">
              <p className="text-xs font-semibold text-icon mb-3">업로드 히스토리</p>
              <div className="space-y-2">
                {leadershipHistory.map(v => (
                  <div key={v.id} className="flex items-center justify-between px-4 py-3 rounded-xl bg-surface-subtle border border-border-light">
                    <div className="flex items-center gap-3">
                      <svg className="w-4 h-4 text-icon flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                      <div>
                        <p className="text-xs font-semibold text-text-primary">{v.fileName}</p>
                        <p className="text-xs text-icon">{v.uploadedAt}</p>
                      </div>
                    </div>
                    <button
        ...
        <div className="bg-surface rounded-2xl border border-border-light shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            {/* 왼쪽: 제목 + 연도 탭 */}
            <div className="flex items-center gap-3">
              <h2 className="text-base font-bold text-text-primary">연도별 직책자 관리</h2>
              <div className="flex items-center gap-1.5">
                {years.map(y => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                      y === selectedYear ? 'bg-brand text-text-onBrand' : 'bg-surface-subtle text-text-secondary hover:bg-gray-200'
                    }`}
                  >
                    {y}년
                  </button>
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold text-icon border border-dashed border-gray-300 hover:border-brand hover:text-brand transition-colors"
                >
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-52 bg-surface border border-border-light rounded-2xl shadow-xl z-20 overflow-hidden p-1.5">
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-text-primary hover:bg-brand/8 hover:text-brand-dark transition-colors group"
                    >
          ...
          <div className="rounded-xl border border-border-light overflow-hidden">
            <div className="grid grid-cols-[2fr_1.2fr_1fr_2fr_1.4fr_0.8fr_88px] px-4 py-2.5 bg-surface-subtle border-b border-border-light">
              {['이름', '부서', '직책', '이메일', '리더십 유형', '발송 회차', ''].map(h => (
                <p key={h} className="text-xs font-semibold text-icon tracking-wider uppercase">{h}</p>
              ))}
            </div>
                ...
                editingId === p.id && editForm ? (
                  <div key={p.id} className="grid grid-cols-[2fr_1.2fr_1fr_2fr_1.4fr_0.8fr_88px] px-4 py-2.5 items-center bg-blue-50/40 gap-2">
                    ...
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={saveEdit} className="text-xs font-semibold text-brand px-2 py-1 rounded hover:bg-blue-100 transition-colors">저장</button>
                      <button onClick={cancelEdit} className="text-xs text-icon px-2 py-1 rounded hover:bg-surface-subtle transition-colors">취소</button>
                    </div>
                  </div>
                ) : (
                  <div key={p.id} className="grid grid-cols-[2fr_1.2fr_1fr_2fr_1.4fr_0.8fr_88px] px-4 py-3 items-center hover:bg-surface-hover transition-colors group">
                    <p className="text-sm font-medium text-text-primary truncate">{p.name}</p>
                    <p className="text-sm text-text-secondary truncate">{p.department}</p>
                    <p className="text-sm text-text-secondary">{p.position}</p>
                    <p className="text-xs text-icon truncate">{p.email}</p>
                    <span className={`inline-flex w-fit text-xs font-semibold px-1.5 h-6 inline-flex items-center rounded ${BADGE_CLS}`}>{p.leadershipType}</span>
                    <p className="text-sm text-text-secondary">{p.assessmentRound}회차</p>
                    <div className="flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => startEdit(p)} className="text-xs text-text-secondary px-2 py-1 rounded hover:bg-gray-200 transition-colors">편집</button>
              ...
              {showAddRow && (
                <div className="grid grid-cols-[2fr_1.2fr_1fr_2fr_1.4fr_0.8fr_88px] px-4 py-2.5 items-center bg-emerald-50/40 gap-2">
                  ...
                  <div className="flex items-center gap-1 justify-end">
                    ...
                    <button onClick={() => setShowAddRow(false)} className="text-xs text-icon px-2 py-1 rounded hover:bg-surface-subtle transition-colors">취소</button>
                  </div>
                </div>
              )}
            </div>
...
const inputCls = 'w-full text-sm text-text-primary border border-border-light rounded-lg px-3 py-2 outline-none focus:border-brand focus:ring-1 focus:ring-brand/20 transition-all';
const rowInputCls = 'w-full text-xs text-text-primary border border-border-light rounded-md px-2 py-1.5 outline-none focus:border-brand transition-all';
