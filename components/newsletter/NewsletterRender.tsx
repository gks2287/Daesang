...
export function renderNewsletterEmailPreview(generated: GeneratedNewsletter, opts: EmailPreviewOpts) {
  const { vol, dateLabel, onReadFull } = opts;
  return (
    <div className="bg-white max-w-md mx-auto rounded-2xl overflow-hidden shadow-sm border border-gray-100 break-keep break-words">
      ...
      <div className="px-5 py-3.5 flex items-center justify-between border-b border-gray-100">
        ...
        <p className="text-[11px] text-[#6B7280]">Vol.{vol}{dateLabel ? ` · ${dateLabel}` : ''}</p>
      </div>
      {/* 헤더 영역 */}
        ...
        <div className="space-y-4">
          {generated.sections.map(sec => (
            <div key={sec.contentId} className="rounded-xl p-4 border border-[#E1EFFB]" style={{ backgroundColor: '#F0F7FF' }}>
              ...
              {onReadFull && (
                <button onClick={onReadFull} className="mt-2 text-xs text-[#2B9EE8] font-bold">
                  자세히 →
                </button>
              )}
            </div>
